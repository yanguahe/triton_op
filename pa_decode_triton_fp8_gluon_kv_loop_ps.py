...
@triton.jit
def pa_decode_v2_gluon_big_blk_fp8_inner_one_q(
    q,
    k,
    k_scale_val,
    v,
...
def pa_decode_v2_gluon_big_blk_fp8(
    ...
    # QID = 0
    # pa_decode_v2_gluon_big_blk_fp8_inner_one_q(
    #     q0,
    #     k,
    #     k_scale_val,
    #     v,
...
def pa_decode_v2_gluon_fp8(
    exp_sums_ptr,       # [num_seqs, num_kv_heads, max_parts, q_grp_sz]
    max_logits_ptr,     # [num_seqs, num_kv_heads, max_parts, q_grp_sz]
    output_ptr,         # [num_seqs, num_kv_heads, max_parts, q_grp_sz, head_sz]
    logits_ptr,         # [num_seqs, num_kv_heads, max_parts, q_grp_sz, head_sz]
    q_ptr,              # [num_seqs, num_kv_heads * query_grp_sz, head_sz]
    k_cache_ptr,        # [num_blks, num_kv_heads, head_sz/x, kv_blk_sz, x]
    v_cache_ptr,        # [num_blks, num_kv_heads, head_sz, kv_blk_sz]
    ...
    # tl.device_print("work_start  ", work_start)
    # tl.device_print("work_end   ", work_end)


    for work_id in range(work_start, work_end):
        seq_idx = tl.load(work_info_set + work_id * 8)
        seq_part_idx = tl.load(work_info_set + work_id * 8 + 1)

        # q_start = tl.load(work_info_set + work_id * 8 + 2) # batch_start
        # q_end = tl.load(work_info_set + work_id * 8 + 3) # batch_end

        kv_seq_start_partial_idx = tl.load(work_info_set + work_id * 8 + 4)
        split_kv_end = tl.load(work_info_set + work_id * 8 + 5)

        kv_seq_len = split_kv_end - kv_seq_start_partial_idx 

        kv_seq_true_start = gl.load(seq_lens_ptr + seq_idx)
        kv_seq_true_end   = gl.load(seq_lens_ptr + seq_idx + 1)

        true_kv_seq_len = kv_seq_true_end - kv_seq_true_start
        kv_seq_start_idx = kv_seq_start_partial_idx - kv_seq_true_start 
        ...
        #     return
        KV_COMPUTE_BLOCK_NUM = kv_seq_len // KV_COMPUTE_BLOCK_SIZE
        # kv_start_block_idx = seq_start_idx  // KV_COMPUTE_BLOCK_SIZE
        # seq_end_idx = gl.minimum(kv_seq_start_idx + SEQ_PARTITION_SZ, kv_seq_len)
        SEQ_PARTITION_NUM_KV_BLKS = kv_seq_len // KV_BLK_SZ
        # tl.device_print("KV_COMPUTE_BLOCK_NUM  ", KV_COMPUTE_BLOCK_NUM)
        ...
        for kv_idx in range(KV_COMPUTE_BLOCK_NUM):
            ...
            kv_blk_start = kv_seq_start_idx // KV_BLK_SZ + kv_idx * MAX_NUM_KV_BLKS
            ...
            if alibi_slopes is not None:
                qk += (alibi_slope[:, None] * (qk_col_offs - true_kv_seq_len + 1)[None, :]).to(gl.float32)

            qk_bound_mask = (qk_row_offs[:, None] < QUERY_GRP_SZ)
            if IS_CAUSAL:
                causal_mask = qk_col_offs[None, :] < true_kv_seq_len - (Q_SEQ_LEN - 1 - QID)
            else:
                causal_mask = qk_col_offs[None, :] < true_kv_seq_len
            qk_bound_mask = qk_bound_mask & causal_mask
            # if [0, SEQ_PARTITION_SZ) are all -inf, the result will be nan
            # so, we use -1e37 other than -inf
            # qk = tl.where(qk_bound_mask, qk, float("-inf"))
            ...
            # if seq_idx == 0:
                # tl.device_print("m_l_grp_mask", m_l_grp_mask)
            # tl.device_print("stride_max_logits_p ", stride_max_logits_p)
        ...
        if seq_part_idx == -1:
            acc = acc0.to(triton.language.bfloat16)
            output_offs = seq_idx * stride_output_s
            output_offs += (
                o_grp_offs[:, None] * stride_output_nh
                + o_head_sz_offs[None, :]
            )
            gl.amd.cdna3.buffer_store(stored_value=acc, ptr=output_ptr, offsets=output_offs + QID * QUERY_GRP_SZ * stride_logits_g, mask=o_mask)
        else:
            # acc0 = acc0.to(COMPUTE_TYPE)
            m_l_offs = (
                seq_part_idx * stride_max_logits_p
            ...
            gl.amd.cdna3.buffer_store(stored_value=m_i + tl.log2(l_i), ptr=exp_sums_ptr, offsets=m_l_offs, mask=m_l_grp_mask)
            # gl.amd.cdna3.buffer_store(stored_value=exp_sum0, ptr=exp_sums_ptr, offsets=m_l_offs + QID * QUERY_GRP_SZ, mask=m_l_grp_mask)
            # gl.amd.cdna3.buffer_store(stored_value=m_i, ptr=max_logits_ptr, offsets=m_l_offs + QID * QUERY_GRP_SZ, mask=m_l_grp_mask)
            # gl.amd.cdna3.buffer_store(stored_value=l_i, ptr=exp_sums_ptr, offsets=m_l_offs + QID * QUERY_GRP_SZ, mask=m_l_grp_mask)
            gl.amd.cdna3.buffer_store(stored_value=acc0, ptr=logits_ptr, offsets=logits_offs + QID * QUERY_GRP_SZ * stride_logits_g, mask=o_mask)
