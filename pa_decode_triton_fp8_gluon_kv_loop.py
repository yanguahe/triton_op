...
def pa_decode_v2_gluon_big_blk_fp8(
    ...
    for kv_idx in range(KV_COMPUTE_BLOCK_NUM):
        ...
        page_id = gl.amd.cdna3.buffer_load(blk_tables_start_ptr + blk_tb_id, offsets=kv_page_id_offs)
        page_offset = kv_sub_seq_start_idx % KV_BLK_SZ
        k_page_id = gl.convert_layout(page_id, layout=blk_layout)
        ...
        if k_temp.dtype.is_fp8():
            if KV_QUANT_MODE == 0:
                ...
                # tl.static_print(kv_scale_page_id.type)
                k_scale_offs = gl.reshape(k_scale_offs, [1, KV_COMPUTE_BLOCK_SIZE])
                ...
                # [1, KV_COMPUTE_BLOCK_SIZE, 1]
                # v_scale_offs = page_id[:, None, None] * kv_scale_stride0 + kv_head_idx * kv_scale_stride1 + (page_offset + blk_offs)[None, :, None]
                v_scale_val = gl.amd.cdna3.buffer_load(ptr=v_scale, offsets=k_scale_offs)
                # v_scale_val = gl.reshape(v_scale_val, [1, KV_COMPUTE_BLOCK_SIZE])
