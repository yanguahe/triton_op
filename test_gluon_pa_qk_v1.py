...
def get_autotune_config():
    sizes = [
        ...
        {'QUERY_GRP_SZ': 8, 'SEQ_PARTITION_SZ': 256, 'SEQ_PARTITION_KV_BLK_NUM': 16, 'K_HD_SPLIT_NUM': 16, 'K_SPLIT_HEAD_SZ': 8, 'KV_BLK_SZ': 16, 'HEAD_SZ': 128},
    ]
...
def gemm_qk(
    ...
    # 1 x QUERY_GRP_SZ x (K_HD_SPLIT_NUM x K_SPLIT_HEAD_SZ)
    ...
    # SEQ_PARTITION_KV_BLK_NUM x (K_HD_SPLIT_NUM x K_SPLIT_HEAD_SZ) x KV_BLK_SZ
    ...
    offs_k_dim1 = gl.arange(0, HEAD_SZ, layout=k_dim_1_layout)
    ...
    for k in range(0, gl.cdiv(k_split_head_sz, HEAD_SZ)):
        # a = gl.amd.cdna3.buffer_load(ptr=q_ptr, offsets=offs_q, mask=offs_q_dim2[None, None, :] < K_HD_SPLIT_NUM)
        # b = gl.amd.cdna3.buffer_load(ptr=k_ptr, offsets=offs_k, mask=offs_k_dim1[None, :, None] < K_HD_SPLIT_NUM)
        a = gl.amd.cdna3.buffer_load(ptr=q_ptr, offsets=offs_q)
        b = gl.amd.cdna3.buffer_load(ptr=k_ptr, offsets=offs_k)
        a_broadcasted = tl.broadcast_to(a, SEQ_PARTITION_KV_BLK_NUM, QUERY_GRP_SZ, HEAD_SZ)
        ...
        q_ptr += HEAD_SZ * q_stride3
        k_ptr += HEAD_SZ * k_stride4
...
def run_gemm_qk(q, k, dtype):
    ...
    q = q.reshape(batch_size, num_kv_heads, QUERY_GRP_SZ, K_HD_SPLIT_NUM * K_SPLIT_HEAD_SZ)
    k = k.transpose(-2, -1).reshape(batch_size, seq_partition_kv_num, SEQ_PARTITION_KV_BLK_NUM, num_kv_heads, K_HD_SPLIT_NUM * K_SPLIT_HEAD_SZ, KV_BLK_SZ)
